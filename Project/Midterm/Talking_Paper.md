...
- "Model 6 with doubled train ratio is currently running — about 55% done, around 1.1 million steps."
...
- "Caveat: these models only ran about 500K steps due to a config error. I plan to rerun both to 2M steps so they're directly comparable."

---

## Slide 10: Remaining Work

- "Model 6 should finish in about four days. Model 7 — larger board — is planned for the first two weeks of April. I also plan to rerun the resolution models to 2M steps."
- "I also want to add collision prediction accuracy — does the world model predict when the snake is about to die? — and test longer imagination horizons beyond the current 15 dreamed steps to see how fast the predictions degrade."
...
**If asked how dreaming works:** "The agent plays in the real environment and stores states in a replay buffer. Then it samples a batch of past states — not necessarily from the current game — and imagines 15 steps forward from each one. The policy trains on those imagined trajectories, then goes back to playing one real step at a time. The dreaming and playing are decoupled."

**If asked about the resolution caveat:** "The 16x16 and 32x32 models only ran about 500K steps due to a config error. I'm planning to rerun both to 2M steps for the final paper so they're directly comparable with the other models."
